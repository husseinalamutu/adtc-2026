...
only, 70 unit tests) supplies every figure. The split is not cosmetic — we *measured* where a
...
**Multilingual output without model-capacity cost.** Hausa, Igbo and Yoruba are rendered from
computed figures by native-authored templates (`i18n.py`), not generated by the model. Figures
are byte-identical across languages by construction, hallucination is structurally impossible in
the language layer, and the model's limited capacity stays spent on English financial reasoning —
which is what the audit measures. Unreviewed catalogues are gated in code and cannot reach a user.
