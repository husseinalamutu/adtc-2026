...
# and imatrix is where we recover most of the accuracy Q4_K_M would otherwise cost (see REPORT.md's
# Design decisions section). We go through llama.cpp's own converter instead (03_to_gguf.sh) so
# imatrix is in the loop.
set -euo pipefail
cd "$(dirname "$0")"
